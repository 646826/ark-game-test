...
export class ConservatoryRenderer {
    ...
    setAnalysis(analysis) { this.#analysis = analysis; }
    setSelected(id) { this.#selectedId = id; }
    setHovered(id) { this.#hoveredId = id; }
    setCoach(id) { this.#coachId = id; }
    setHint(id, durationMs = 5_000) { this.#hintId = id; this.#hintUntil = performance.now() + durationMs; }
    clearHint() { this.#hintId = null; }
    setReducedMotion(enabled) { this.#reducedMotion = enabled; }
    setHighContrast(enabled) { this.#highContrast = enabled; }
    setQuality(quality) { this.#quality = quality; this.#createDust(); }
    rotateView(delta) { this.#targetViewTurns += delta; if (this.#reducedMotion)
        this.#viewTurns = this.#targetViewTurns; }
    syncTile(tile) { if (!this.#displayTurns.has(tile.id))
        this.#displayTurns.set(tile.id, tile.visualTurns); }
    ...
    #drawTile(projected, theme, time) {
        ...
        if (tile.kind === 'source')
            ...
            this.#drawPlant(projected, { x: center.x, y: center.y - lift }, theme, powered, time);
        else
            this.#drawMechanism(projected, { x: center.x, y: center.y - lift }, theme, powered, time);
        if (tile.fixed && tile.kind === 'pipe')
            this.#drawLock(center.x + tileWidth * 0.27, center.y - lift - tileHeight * 0.10, tileWidth * 0.08, theme);
        if (selected || hovered || coached || hinted)
            this.#drawSelection(topPolygon, tileWidth, theme, time, coached || hinted);
        context.restore();
        if (this.#analysis) {
            for (const leak of this.#analysis.leaks) {
                if (leak.tileId === tile.id)
                    this.#drawLeak(projected, { x: center.x, y: center.y - lift }, leak.direction, theme, time);
            }
        }
    }
    ...
    #drawChannels(projected, center, theme, powered, time) {
        const context = this.#context;
    ...
    #drawMechanism(projected, center, theme, powered, time) {
        const context = this.#context;
    ...
    #drawPlant(projected, center, theme, powered, time) {
        const context = this.#context;
    ...
    #drawSelection(polygon, tileWidth, theme, time, strong) {
        const context = this.#context;
