...
const CORE_PREMIUM_SPRITES = ['leak', 'lock'];
const CINEMATIC_SPRITES = new Set([
    'coupler',
    'leak',
    'lock',
    'mechanism-terminal',
...
const PLATFORM_SPRITES = ['platform-0', 'platform-1', 'platform-2', 'platform-3'];
const MECHANISM_SPRITES = ['mechanism-terminal', 'mechanism-straight', 'mechanism-elbow', 'mechanism-junction'];
function platformSpriteKey(tile) {
    const variant = (tile.x * 3 + tile.y * 5 + tile.baseMask) & 3;
    return PLATFORM_SPRITES[variant] ?? 'platform-0';
}
function mechanismVariant(tile) {
    const ports = DIRECTIONS.filter((direction) => (tile.baseMask & direction.bit) !== 0);
    const oppositePair = ports.length === 2 &&
...
function mechanismSpriteKey(tile) {
...
export class ConservatoryRenderer {
    ...
    #staticCanvas;
    #staticContext;
    #art = new ConservatoryArtCache();
    #sprites = new Map();
    #readySprites = new Set();
    ...
    #reducedMotion = false;
    #highContrast = false;
    #profile = resolveRenderProfile('auto', detectDeviceSignals());
    ...
    constructor(canvas) {
        ...
        if (!staticContext)
            throw new Error('Canvas 2D static cache is unavailable.');
    ...
    setRenderProfile(profile) {
        ...
        if (changed) {
            this.#art.clear();
            if (profile.quality === 'high' && this.#puzzle)
                this.#ensurePremiumSprites(this.#premiumSpritesForPuzzle(this.#puzzle));
            this.#createDust();
            this.#staticDirty = true;
            this.resize();
            this.#invalidate(700);
    ...
    rotateView(delta) {
        ...
        if (this.#reducedMotion || this.#canUseStaticCache())
            this.#viewTurns = this.#targetViewTurns;
        this.#projectionDirty = true;
        this.#staticDirty = true;
    ...
    syncTile(tile) {
        if (!this.#displayTurns.has(tile.id) || this.#canUseStaticCache())
            this.#displayTurns.set(tile.id, tile.visualTurns);
        this.#staticDirty = true;
    ...
    #premiumSpritesForPuzzle(puzzle) {
        const keys = new Set(['coupler', 'leak', 'pipe']);
        for (const tile of puzzle.tiles) {
            keys.add(platformSpriteKey(tile));
            if (tile.kind === 'source')
                keys.add('source');
            if (tile.kind === 'pipe')
                keys.add(mechanismSpriteKey(tile));
            if (tile.fixed)
                keys.add('lock');
    ...
    #canUseStaticCache() {
        if (this.#profile.quality !== 'high' || !this.#puzzle)
            return false;
    ...
    #recordDrawCost(cost, time, underLoad) {
        ...
        if (!eligibleLoadSample || !this.#profile.adaptive || this.#profile.quality !== 'high' || this.#adaptiveDowngraded || time - this.#profileAppliedAt < 4_500)
            return;
    ...
    #draw(time) {
        ...
        if (this.#canUseStaticCache()) {
            if (this.#staticDirty)
                this.#rebuildStaticBoard(theme, time);
    ...
    #drawLiveBoardEffects(theme, time) {
        if (!this.#analysis || !this.#puzzle)
            return;
        const context = this.#context;
        const byPosition = new Map(this.#projected.map((projected) => [`${projected.tile.x},${projected.tile.y}`, projected]));
        ...
        for (const projected of this.#projected) {
            ...
            if (normalizedPower > 0.018) {
                ...
                for (const direction of DIRECTIONS) {
                    if ((tile.baseMask & direction.bit) === 0)
                        continue;
                ...
                if (!this.#reducedMotion && tile.kind === 'pipe') {
                    ...
                    context.beginPath();
                    context.arc(projected.center.x + Math.cos(phase) * orbitRadius, coreY + Math.sin(phase) * orbitRadius * 0.42, Math.max(1.1, projected.tileWidth * 0.0065), 0, Math.PI * 2);
                    context.fill();
                }
        ...
        for (const projected of this.#projected) {
            ...
            if (!this.#analysis.powered.has(projected.tile.id))
                continue;
            for (const direction of DIRECTIONS) {
                if ((mask & direction.bit) === 0)
                    continue;
                const neighbor = byPosition.get(`${projected.tile.x + direction.dx},${projected.tile.y + direction.dy}`);
                if (!neighbor || projected.tile.id >= neighbor.tile.id || !this.#analysis.powered.has(neighbor.tile.id))
                    continue;
                if ((currentMask(neighbor.tile) & direction.opposite) === 0)
                    continue;
    ...
    #drawConnectionBridges(theme, time) {
        if (this.#profile.quality !== 'high' || !this.#puzzle || !this.#analysis)
            return;
        ...
        for (const projected of this.#projected) {
            ...
            for (const direction of DIRECTIONS) {
                if ((mask & direction.bit) === 0)
                    continue;
                const neighbor = byPosition.get(`${projected.tile.x + direction.dx},${projected.tile.y + direction.dy}`);
                if (!neighbor || projected.tile.id >= neighbor.tile.id)
                    continue;
                if ((currentMask(neighbor.tile) & direction.opposite) === 0)
                    continue;
                const dx = neighbor.center.x - projected.center.x;
    ...
    #drawTile(projected, theme, time) {
        ...
        if (cinematicPlatform) {
            ...
            context.drawImage(cinematicPlatform, center.x - 512 * platformScaleX, center.y - lift - 276 * platformScaleY, cinematicPlatform.naturalWidth * platformScaleX, cinematicPlatform.naturalHeight * platformScaleY);
        }
        else {
            ...
            context.drawImage(platform.image, center.x - platform.dimensions.anchorX * platformScaleX, center.y - lift - platform.dimensions.anchorY * platformScaleY, platform.dimensions.width * platformScaleX, platform.dimensions.height * platformScaleY);
        }
        ...
        if (premiumPlate) {
            ...
            context.drawImage(premiumPlate, center.x - 256 * spriteScaleX, center.y - lift - 174 * spriteScaleY, 512 * spriteScaleX, 512 * spriteScaleY);
            context.restore();
        }
        ...
        if (powered || this.#highContrast) {
            ...
            context.clip();
            const normalizedPower = clamp(power, 0, 1);
            const glow = context.createRadialGradient(center.x - tileWidth * 0.14, center.y - lift - tileHeight * 0.20, 0, center.x, center.y - lift, tileWidth * 0.55);
        ...
        if (!this.#renderingStatic && (selected || hovered || coached || hinted || pressed))
        ...
        for (const direction of DIRECTIONS) {
            ...
            if (coupler) {
                ...
                if (powered) {
                    ...
                    context.beginPath();
                    context.ellipse(end.x, end.y, width * (0.28 + normalizedPower * 0.16), width * (0.18 + normalizedPower * 0.10), Math.atan2(vector.y, vector.x), 0, Math.PI * 2);
                    context.fill();
                }
                context.restore();
            }
    ...
    #drawMechanism(projected, center, theme, power, time) {
        ...
        if (premiumMechanism) {
            ...
            context.drawImage(premiumMechanism, center.x - spriteSize / 2, center.y - spriteSize * 0.515, spriteSize, spriteSize);
            context.restore();
        }
        else {
            ...
            context.rotate(spin);
            context.drawImage(art.image, -art.dimensions.anchorX * scale, -art.dimensions.anchorY * scale, art.dimensions.width * scale, art.dimensions.height * scale);
            context.restore();
        }
        if (this.#renderingStatic && premiumMechanism)
            return;
        ...
        const core = context.createRadialGradient(center.x - radius * 0.25, center.y - radius * 0.32, 0, center.x, center.y, radius * 0.65);
    ...
    #drawSource(projected, center, theme, time) {
        ...
        if (premiumSource) {
            ...
            context.drawImage(premiumSource, center.x - premiumSize / 2, baseY - premiumSize * 0.92, premiumSize, premiumSize);
        }
        else {
            context.drawImage(art.image, center.x - art.dimensions.anchorX * scale, baseY - art.dimensions.anchorY * scale, art.dimensions.width * scale, art.dimensions.height * scale);
        }
        context.restore();
        if (this.#renderingStatic && premiumSource)
            return;
        ...
        const orb = context.createRadialGradient(center.x - radius * 0.32, orbY - radius * 0.38, 0, center.x, orbY, radius * 0.92);
            ...
            if (offSprite && onSprite) {
                ...
                if (!this.#reducedMotion && normalizedPower > 0.35) {
                    ...
                    for (let index = 0; index < 6; index += 1) {
                        ...
                        context.beginPath();
                        context.arc(center.x + Math.cos(angle) * projected.tileWidth * (0.035 + phase * 0.08), potBaseY - projected.tileHeight * (0.25 + phase * 0.52) + Math.sin(angle) * projected.tileHeight * 0.035, Math.max(0.8, projected.tileWidth * 0.006), 0, Math.PI * 2);
                        context.fill();
                    }
                    context.restore();
                }
    ...
    #drawDetailedBloom(x, y, radius, kind, bloom, time, power) {
        const context = this.#context;
        const palette = PLANT_PALETTES[kind];
