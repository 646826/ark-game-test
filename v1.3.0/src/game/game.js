...
export class ClockworkGame {
    ...
    #renderMap() {
        ...
        nodes.forEach((node) => {
            ...
            const dome = node.querySelector('[data-dome-image]');
            if (dome)
                dome.src = `./assets/hd/dome-${state}.webp`;
    ...
    #renderSpecimens(scope) {
        ...
        cards.forEach((card) => {
            ...
            const image = card.querySelector('[data-specimen-image]');
            if (image) {
    ...
    #renderHud() {
        ...
        const bloomCheck = this.#gameHud.querySelector('[data-side-bloom-check]');
        if (bloomCheck) {
        ...
        const leakCheck = this.#gameHud.querySelector('[data-side-leak-check]');
        if (leakCheck) {
            const complete = analysis.leaks.length === 0;
            leakCheck.textContent = complete ? '✓' : '○';
            leakCheck.dataset.complete = String(complete);
        }
        const moveCheck = this.#gameHud.querySelector('[data-side-move-check]');
        if (moveCheck) {
        ...
        setText(this.#gameHud, '[data-restoration-stage]', stageNames[Math.min(stageNames.length - 1, Math.floor(restoration / 20))]);
        for (const restorationRing of this.#gameHud.querySelectorAll('.restoration-ring')) {
        ...
        for (const dailyProgress of this.#gameHud.querySelectorAll('[data-daily-progress]')) {
            dailyProgress.style.setProperty('--daily', `${Math.round((dailyCount / dailyTarget) * 100)}%`);
        }
        for (const flow of this.#gameHud.querySelectorAll('[data-flow-meter]')) {
        ...
        for (const hintButton of this.#gameHud.querySelectorAll('[data-action="hint"]')) {
            hintButton.disabled = this.#hintBusy;
        }
        for (const undoButton of this.#gameHud.querySelectorAll('[data-action="undo"]')) {
