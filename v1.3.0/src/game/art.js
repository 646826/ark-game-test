...
function canvas(width, height) {
    ...
    if (!context)
        throw new Error('Canvas 2D is unavailable for art cache.');
    context.imageSmoothingEnabled = true;
    if ('imageSmoothingQuality' in context)
        context.imageSmoothingQuality = 'high';
    return [element, context];
}
function drawPlatform(palette, quality, variant) {
    const dimensions = { width: 620, height: 440, anchorX: 310, anchorY: 178 };
    ...
    for (const [x, y] of sideDots)
        drawScrew(context, x, y, 8, palette.brassLight, palette.brass);
    return { image, dimensions };
}
function drawMechanism(palette, quality, variant) {
    const dimensions = { width: 360, height: 360, anchorX: 180, anchorY: 186 };
    const [image, context] = canvas(dimensions.width, dimensions.height);
    ...
    context.lineWidth = 4;
    context.stroke();
    const toothCounts = [18, 12, 10, 8];
    const toothCount = toothCounts[type];
    context.save();
    ...
    for (let index = 0; index < toothCount; index += 1) {
        ...
        if (type === 0) {
            ...
            context.stroke();
            if (alternating)
                drawScrew(context, radius * 1.08, 0, 4.6, palette.brassLight, '#5a3218');
        }
        context.restore();
    }
    context.restore();
    ...
    if (type === 0) {
        ...
        context.stroke();
        for (const x of [-radius * 0.55, radius * 0.55])
            drawScrew(context, x, 0, 6, palette.brassLight, palette.brass);
    }
    else if (type === 2) {
        context.strokeStyle = palette.brassLight;
...
function drawSourceBase(palette, quality) {
    const dimensions = { width: 420, height: 430, anchorX: 210, anchorY: 250 };
    const [image, context] = canvas(dimensions.width, dimensions.height);
    const cx = dimensions.anchorX;
    const cy = dimensions.anchorY;
    const radius = 88;
...
function drawPlanter(palette, quality, kind) {
    const dimensions = { width: 340, height: 420, anchorX: 170, anchorY: 312 };
    const [image, context] = canvas(dimensions.width, dimensions.height);
    ...
    if (quality === 'high' && 'filter' in context)
    ...
    for (const offset of [-0.25, 0, 0.25])
        drawScrew(context, cx + potW * offset, potY + potH * 0.43, 5, palette.brassLight, palette.brass);
    ...
    for (let index = 0; index < 18; index += 1) {
        ...
        leaf.addColorStop(0, leafColors[index % leafColors.length]);
        leaf.addColorStop(0.52, leafColors[(index + 2) % leafColors.length]);
...
function drawCornerPlate(context, x, y, palette, quality) {
    context.save();
...
function drawMetalRing(context, cx, cy, outer, inner, palette, quality) {
...
function drawScrew(context, x, y, radius, light, dark) {
...
function points(cx, cy, width, height) {
    return [
        { x: cx, y: cy - height / 2 },
        { x: cx + width / 2, y: cy },
        { x: cx, y: cy + height / 2 },
...
function insetPoints(cx, cy, width, height) {
