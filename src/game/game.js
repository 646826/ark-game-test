...
import { isTutorialSeed, tutorialForLevel } from '../core/tutorial.js';
import { I18n, detectLanguage, isSupportedLanguage } from '../ui/i18n.js';
...
export class ClockworkGame {
    ...
    constructor(app, bridge, version) {
        ...
        this.#canvas = mustElement(app, '#game-canvas');
        ...
        this.#modal = mustElement(app, '#game-modal');
    ...
    async #startMode(mode, resumePreferred) {
        ...
        const canResume = resumePreferred &&
            snapshot !== undefined &&
    ...
    #restoreSnapshot(snapshot) {
        ...
        if (snapshot.rotations.length === puzzle.tiles.length) {
            puzzle.tiles.forEach((tile, index) => {
                ...
                const rotation = snapshot.rotations[index] % period;
    ...
    #rotateTile(tileId) {
        ...
        this.#announce(this.#i18n.t('tileRotated', {
            row: tile.y + 1,
            column: tile.x + 1,
        ...
        if (this.#analysis.solved && !this.#completionPending) {
            ...
            this.#completionPending = true;
            window.setTimeout(() => void this.#completePuzzle(), this.#progress.settings.reducedMotion ? 80 : 620);
        }
    }
    #undo() {
        const puzzle = this.#puzzle;
    ...
    #renderCompletion(stats, dailyIsBest, leaderboardPosted) {
        const puzzle = this.#puzzle;
        if (!puzzle) {
            return;
        }
        setText(this.#completionOverlay, '[data-completion-title]', this.#i18n.t(puzzle.mode === 'daily' ? 'dailyCompleteTitle' : 'completeTitle'));
        setText(this.#completionOverlay, '[data-completion-grade]', this.#i18n.t(stats.stars === 3 ? 'perfectCircuit' : stats.stars === 2 ? 'gracefulCircuit' : 'restoredCircuit'));
        ...
        setText(this.#completionOverlay, '[data-leaderboard-status]', puzzle.mode === 'daily' ? this.#i18n.t(leaderboardPosted ? 'leaderboardPosted' : 'leaderboardOffline') : '');
        const unlocked = puzzle.mode === 'campaign' ? specimenUnlockedAt(puzzle.level) : null;
        setText(this.#completionOverlay, '[data-unlock-message]', unlocked ? this.#i18n.t('specimenUnlocked', { specimen: this.#specimenName(unlocked) }) : '');
        const nextButton = mustElement(this.#completionOverlay, '[data-action="next"]');
    ...
    #moveSelection(key) {
        ...
        const direction = key === 'arrowup' ? { x: 0, y: -1 } : key === 'arrowdown' ? { x: 0, y: 1 } : key === 'arrowleft' ? { x: -1, y: 0 } : { x: 1, y: 0 };
    ...
    #renderMenu() {
        const continueButton = mustElement(this.#menuOverlay, '[data-action="continue-run"]');
        ...
        setText(this.#menuOverlay, '[data-restoration-count]', this.#i18n.t('circuitsRestored', { count: restored }));
        setText(this.#menuOverlay, '[data-chamber-progress]', this.#i18n.t('chamberProgress', { current: chamberProgress, total: 5 }));
        const progressBar = mustElement(this.#menuOverlay, '[data-restoration-bar]');
        progressBar.style.setProperty('--restoration-progress', `${(chamberProgress / 5) * 100}%`);
        this.#menuOverlay.querySelectorAll('[data-specimen]').forEach((element) => {
            const kind = element.dataset.specimen;
            if (!kind) {
                return;
            }
            const unlocked = restored >= specimenUnlockLevel(kind);
            element.dataset.unlocked = String(unlocked);
            element.setAttribute('aria-label', unlocked
    ...
    #renderHud() {
        ...
        const leakStat = mustElement(this.#hud, '.leak-stat');
        ...
        const hintButton = mustElement(this.#toolbar, '[data-action="hint"]');
        ...
        const undoButton = mustElement(this.#toolbar, '[data-action="undo"]');
        undoButton.disabled = this.#history.length === 0 || this.#completionPending;
        const soundButton = mustElement(this.#toolbar, '[data-action="toggle-sound"]');
    ...
    #renderCoach() {
        ...
        setText(this.#coachOverlay, '[data-coach-title]', this.#i18n.t(`tutorial${step}Title`));
        setText(this.#coachOverlay, '[data-coach-body]', this.#i18n.t(`tutorial${step}Body`));
        const legend = mustElement(this.#coachOverlay, '[data-coach-legend]');
        legend.innerHTML =
            step === 1
    ...
    #specimenName(kind) {
        return this.#i18n.t(`specimen${kind[0]?.toUpperCase()}${kind.slice(1)}`);
    }
    #renderCanvasLabel() {
        const puzzle = this.#puzzle;
        const analysis = this.#analysis;
        ...
        this.#canvas.setAttribute('aria-label', `${this.#i18n.t('title')}. ${kind}, ${this.#i18n.t('level')} ${puzzle.level}, row ${tile.y + 1}, column ${tile.x + 1}, ${state}, ${fixed}. ${this.#i18n.t('rotateInstruction')}`);
    }
    #openHelp() {
        this.#modalKind = 'help';
        this.pause('modal');
    ...
    #openSettings(preserveOpen = false) {
    ...
    #showModal() {
        ...
        requestAnimationFrame(() => this.#modal.querySelector('button, select, input')?.focus());
    }
    #closeModal() {
        this.#modalKind = null;
        if (this.#modal.open) {
    ...
    #applyTranslations() {
        ...
        this.#app.querySelectorAll('[data-i18n-aria]').forEach((element) => {
    ...
    #chapterName(level) {
        const keys = ['chapterDawn', 'chapterVerdant', 'chapterTwilight', 'chapterAurora'];
        return this.#i18n.t(keys[Math.floor((Math.max(1, level) - 1) / 5) % keys.length]);
    }
    #modeName(mode) {
    ...
    #scheduleSave(delayMs = 450) {
        window.clearTimeout(this.#saveTimer);
        this.#saveTimer = window.setTimeout(() => void this.#saveNow(), delayMs);
    }
    async #saveNow(captureActiveRun = true) {
    ...
    #showToast(message, durationMs = 3_200) {
        window.clearTimeout(this.#toastTimer);
...
function specimenUnlockLevel(kind) {
    return SPECIMEN_UNLOCKS[kind];
}
function specimenUnlockedAt(level) {
    const match = Object.entries(SPECIMEN_UNLOCKS).find(([, unlockLevel]) => unlockLevel === level);
    return match?.[0] ?? null;
}
