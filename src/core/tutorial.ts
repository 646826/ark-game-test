...
import { Direction, type DifficultyConfig, type PlantKind, type PuzzleDefinition, type TileKind, type TileState } from './types.js';
...
export function tutorialForLevel(level: number): TutorialPuzzle | null {
  if (level === 1) {
...
export function isTutorialSeed(seed: string): boolean {
...
function config(cols: number, rows: number, activeCells: number, plants: number): DifficultyConfig {
  return {
    tier: 0,
    cols,
    rows,
...
function rotationForMask(baseMask: number, desiredMask: number): number {
