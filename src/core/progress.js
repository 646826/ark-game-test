...
export function createDefaultProgress(language) {
    return {
        schemaVersion: 1,
        campaignLevel: 1,
        totalScore: 0,
...
export function sanitizeProgress(value, fallbackLanguage) {
    ...
    if (isRecord(activeCandidate)) {
        ...
        if (isRecord(configCandidate) && Array.isArray(rotationsCandidate)) {
            const mode = activeCandidate.mode === 'daily' || activeCandidate.mode === 'zen' || activeCandidate.mode === 'campaign'
            ...
            if (mode && typeof activeCandidate.seed === 'string') {
                result.activeRun = {
                    ...
                    rotations: rotationsCandidate.slice(0, 144).map((rotation) => safeInteger(rotation, 0, 0, 3)),
...
export function serializedSize(progress) {
    return new TextEncoder().encode(JSON.stringify(progress)).byteLength;
}
function sanitizeDailyScores(value) {
    if (!isRecord(value)) {
        return {};
    }
    const entries = Object.entries(value)
        ...
        .slice(-90)
        .map(([key, score]) => [key, Math.max(0, Math.round(score))]);
