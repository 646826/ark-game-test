export class AudioEngine {
    ...
    connect(strength = 1) {
        ...
        if (lift >= 2)
            window.setTimeout(() => this.#tone(1046.5, 0.16, 'sine', 0.08, 1174.66), 118);
    }
    ...
    #scheduleAmbient() {
        window.clearTimeout(this.#ambientTimer);
        if (!this.#context || !this.#music || !this.#musicEnabled || !this.#unlocked || this.#suspended || this.#context.state !== 'running')
            return;
        const chords = [
            [130.81, 196, 261.63],
            [146.83, 220, 293.66],
