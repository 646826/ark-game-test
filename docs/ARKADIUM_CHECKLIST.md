...
|---|---|---|
| Official Game SDK | Dynamically loads SDK v2 from Arkadium CDN; 1.8 s non-blocking fallback | Implemented |
| Ready signal | Calls `lifecycle.onTestReady()` after menu is interactive | Implemented |
...
| Ads | Interstitial at a natural break; player-triggered rewarded hint | Implemented |
| Analytics | Standard events/page views/errors; release provider reads `arkadium-app-insights-id` from HTML | Implemented; production app ID required |
...
| Initial package under 15 MB | Build report measures about 169 KB uncompressed | Pass |
| Total package under 100 MB | Build report measures about 318 KB including maps/report | Pass |
...
| AI latency | Local computation plus visible thinking UI; tested on 100 generated levels | Pass |
| Responsive 2:1 to 1:2 | CSS/canvas resize continuously; desktop and 390×844 smoke screenshots | Pass locally |
...
| Public version number | `0.1.0` visible on title screen and in build report | Pass |

## Required before submission

- Deploy `dist/` to a stable HTTPS URL.
- Run the public URL through the Arkadium Sandbox and confirm every lifecycle status indicator.
...
- Confirm the desired interstitial cadence and rewarded-ad behavior with the producer.
