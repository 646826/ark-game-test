...
export function generatePuzzle(options) {
    ...
    while (stack.length > 0) {
        ...
        const candidates = random.shuffle(DIRECTIONS.filter((direction) => {
    ...
    for (let index = 0; index < loopAttempts; index += 1) {
        ...
        const a = key(x, y);
        const b = key(nx, ny);
        if (((masks.get(a) ?? 0) & direction.bit) !== 0)
            continue;
    ...
    const tiles = positions.map((position, index) => {
        ...
        if (!fixed && period > 1 && random.next() < 0.74 && rotation === 0)
            rotation = random.integer(1, period - 1);
        return {
            id,
            x: position.x,
            y: position.y,
            ...
            fixed,
            ...(isPlant ? { plantKind: PLANTS[(level + index) % PLANTS.length] } : {}),
            solutionRotation,
            rotation,
