...
const PLANTS = ['lumen', 'orchid', 'starbell', 'ember', 'moonfern'];
export function dailySeed(date = new Date()) {
    return `daily:${date.toISOString().slice(0, 10)}`;
}
export function generatePuzzle(options) {
    if (options.mode === 'campaign' && options.level <= 3)
        return tutorialPuzzle(options.level);
    ...
    for (const candidate of rng.shuffle([...all])) {
        if (active.size <= Math.max(8, config.width * config.height - config.holes))
            break;
        if (candidate.x === Math.floor(config.width / 2) && candidate.y === 0)
            continue;
        active.delete(`${candidate.x},${candidate.y}`);
        if (!isConnected(active))
            active.set(`${candidate.x},${candidate.y}`, candidate);
    }
    ...
    const masks = new Map(cells.map((cell) => [`${cell.x},${cell.y}`, 0]));
    ...
    if (leaves.length < 2)
        leaves = cells.filter((cell) => key(cell) !== key(source)).slice(-2);
    ...
    const tiles = cells.map((cell, index) => {
        ...
        if (!fixed && rotation === 0 && rng.next() < 0.72)
            rotation = 1 % period;
        return {
...
function randomizedTree(active, start, rng) {
    const visited = new Set([key(start)]);
    const edges = [];
    const frontier = neighbors(start, active).map(([cell, direction]) => [start, cell, direction]);
    while (frontier.length > 0) {
        ...
        for (const [next, nextDirection] of neighbors(to, active)) {
            if (!visited.has(key(next)))
                frontier.push([to, next, nextDirection]);
        }
    }
    return edges;
}
function neighbors(cell, active) {
    const result = [];
    for (const direction of DIRECTIONS) {
        const [dx, dy] = DELTA[direction];
        const neighbor = active.get(`${cell.x + dx},${cell.y + dy}`);
        if (neighbor)
            result.push([neighbor, direction]);
    }
    return result;
}
function isConnected(active) {
...
function bitCount(mask) { let value = mask; let count = 0; while (value) {
    count += value & 1;
    value >>>= 1;
} return count; }
export function tutorialPuzzle(level) {
    ...
    const tiles = definitions[level].map((entry, index) => ({
