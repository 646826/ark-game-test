export function difficultyFor(level, mode) {
    if (mode === 'zen') {
    ...
    if (level <= 3)
        return { width: 3, height: 3, holes: 0, fixedRatio: 0, tier: 'Seedling' };
    if (level <= 8)
        return { width: 4, height: 4, holes: 1, fixedRatio: 0.03, tier: 'Gardener' };
    if (level <= 18)
        return { width: 5, height: 5, holes: 2, fixedRatio: 0.08, tier: 'Botanist' };
